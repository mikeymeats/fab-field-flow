...
export default function Omnibar(){
  ...
  if(!open) return null
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-start justify-center p-8" onClick={()=>setOpen(false)}>
      <div className="w-full max-w-2xl bg-neutral-950 border border-neutral-800 rounded-xl overflow-hidden" onClick={(e)=>e.stopPropagation()}>
        <input autoFocus value={q} onChange={e=>setQ(e.target.value)} placeholder="Search hangers, packages, assignments…"
          className="w-full px-3 py-3 bg-transparent outline-none border-b border-neutral-800"/>
        <div className="max-h-80 overflow-auto">
          {items.map((it, i)=>(
            <button key={i} onClick={()=>{ nav(it.to); setOpen(false) }} className="w-full text-left px-3 py-2 hover:bg-white/5 border-b border-neutral-900/40">
              {it.label}
            </button>
          ))}
          {items.length===0 && <div className="px-3 py-8 text-sm opacity-70">No results.</div>}
        </div>
      </div>
    </div>
  )
